...
# -----------------------------
# Step 4: Multi-frame setup
# -----------------------------
face_frame_count = {}  # key=name, value=frames_seen
frames_required = 5    # number of consecutive frames required to mark attendance
...
while True:
    ...
    for face_encoding, face_location in zip(face_encodings, face_locations):
        ...
        # -----------------------------
        # Multi-frame attendance
        # -----------------------------
        if name != "Unknown":
            if name in face_frame_count:
                face_frame_count[name] += 1
            else:
                face_frame_count[name] = 1

            # Mark attendance only after enough consecutive frames
            if face_frame_count[name] == frames_required:
    ...
    # Reset counters for faces not detected in current frame
    for n in list(face_frame_count.keys()):
        if n not in detected_names:
            face_frame_count[n] = 0
